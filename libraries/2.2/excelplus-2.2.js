...
ExcelPlus.prototype = {
    ...
    */
        ...
        if (typeof FileReader === "function") {
            ...
            addEventListener(document.getElementById(params.idButton), "change", function(e) {
                ...
                fileReader.onload = function(e) {
                    ...
                    _this.oFile = XLSX.read(e.target.result, {
                        type: 'binary',
                        cellDates: true,
                        cellStyles: true
                    });
                ...
                if (window.FileReader && window.FileReader.prototype.readAsArrayBuffer) {
                    d.readAsArrayBuffer(file);
                } else {
                    d.readAsBinaryString(file);
                }
            });
        } else {
            Flash.getButtonLabel = function() {
                return params.labelButton
            };
    ...
    openRemote: function(url, onReady) {
        ...
        var convertResponseBodyToText = function(e) {
            return e
        };
        ...
        if (this._isRange(range)) {
            ...
            for (R = range.s.r; R <= range.e.r; ++R) {
                ...
                for (C = range.s.c; C <= range.e.c; ++C) {
                    line.push(this.read(XLSX.utils.encode_cell({
                        c: C,
                        r: R
                    }), options));
                }
    ...
    saveAs: function(filename) {
        ...
        var wbout = XLSX.write(this.oFile, {
            bookType: ext,
            bookSST: true,
            type: 'binary'
        });
    ...
    _updateUsedRange: function(coord) {
        ...
        coord.column -= 1;
        var anyChange = false
        if (range.s.r > coord.row) {
            anyChange = true;
            range.s.r = coord.row;
        }
        if (range.s.c > coord.column) {
            anyChange = true;
            range.s.c = coord.column;
        }
        if (range.e.r < coord.row) {
            anyChange = true;
            range.e.r = coord.row;
        }
        if (range.e.c < coord.column) {
            anyChange = true;
            range.e.c = coord.column;
        }
    ...
    _getColorIndex: function(color) {
        ...
        switch (color) {
            ...
            case "orange":
                return 45;
            default:
                this.error = "[_getColorIndex] '" + color + "' is an unknown color. Known colors are: black, white, red, green, blue, yellow, magenta and cyan";
                this._showErrors();
                return this;
        }
    },
     ...
     */
    _isRange: function(range) {
        return (typeof range === "string" && range.indexOf(":") != -1 && range.match(/[1-9]/) != null);
    },
    /**
     * @ignore
     ...
     */
    _isColumn: function(range) {
        return (typeof range === "string" && range.match(/[1-9]/) == null);
    },
    /**
     * @ignore
            ...
            var w = this,
                ...
                E = function() {
                    ...
                    if (x) {
                        ...
                        if (navigator.userAgent.match(/7\.[\d\s\.]+Safari/) && typeof window.FileReader !== "undefined" && t.size <= 1024 * 1024 * 150) {
                            var G = new window.FileReader();
                            G.readAsDataURL(t);
                            G.onloadend = function() {
                                var H = r.createElement("iframe");
                                H.src = G.result;
                            ...
                            w.readyState = w.DONE;
                            w.savedAs = w.SAVEDASUNKNOWN;
                            return
                        } else {
                            window.open(y, "_blank");
                            w.readyState = w.DONE;
                            w.savedAs = w.SAVEDASBLOB;
                            B();
                            return
                        }
                    }
...
var Flash = {
    ...
    _waitForObjectToBeLoaded: function(id, callback) {
        if (swfobject == undefined) setTimeout(function() {
            Flash._waitForObjectToBeLoaded(id, callback)
        }, 50)
